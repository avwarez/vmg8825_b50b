/* Shared library add-on to iptables to add connzextmark matching support.
 *
                  ...
                  const void *entry, struct xt_entry_match **match)
{
	...
	switch (c) {
	case '1': /* --mark */
		xtables_param_act(XTF_ONLY_ONCE, "connzextmark", "--mark", *flags & F_MARK);
		if (!xtables_strtoui(optarg, &end, &mark, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "connzextmark", "--mark", optarg);
		if (*end == '/')
...
static void connmark_mt_check(unsigned int flags)
{
	if (flags == 0)
		xtables_error(PARAMETER_PROBLEM,
		           "connzextmark: The --mark option is required");
}
