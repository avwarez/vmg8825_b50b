/* Shared library add-on to iptables to add CONNZEXTMARK target support.
 *
	...
	case '1':
		...
		if (*flags)
			xtables_error(PARAMETER_PROBLEM,
			           "CONNZEXTMARK target: Can't specify --set-mark twice");
		*flags = 1;
		break;
	case '2':
		...
		if (*flags)
			xtables_error(PARAMETER_PROBLEM,
			           "CONNZEXTMARK target: Can't specify --save-mark twice");
		*flags = 1;
		break;
	case '3':
		...
		if (*flags)
			xtables_error(PARAMETER_PROBLEM,
			           "CONNZEXTMARK target: Can't specify --restore-mark twice");
		*flags = 1;
		break;
	case '4':
		if (!*flags)
			xtables_error(PARAMETER_PROBLEM,
			           "CONNZEXTMARK target: Can't specify --mask without a operation");
                             ...
                             struct xt_entry_target **target)
{
	...
	case '=': /* --set-xmark */
	case '-': /* --set-mark */
		xtables_param_act(XTF_ONE_ACTION, "CONNZEXTMARK", *flags & F_MARK);
		if (!xtables_strtoui(optarg, &end, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNZEXTMARK", "--set-xmark/--set-mark", optarg);
		if (*end == '/')
	...
	case '&': /* --and-mark */
		...
		if (!xtables_strtoui(optarg, NULL, &mask, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNZEXTMARK", "--and-mark", optarg);
		info->mode   = XT_CONNMARK_SET;
		info->ctmark = 0;
		info->ctmask = ~mask;
		*flags      |= F_MARK;
	...
	case '|': /* --or-mark */
		...
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNZEXTMARK", "--or-mark", optarg);
		info->mode   = XT_CONNMARK_SET;
		info->ctmark = value;
		info->ctmask = value;
		*flags      |= F_MARK;
	...
	case '^': /* --xor-mark */
		xtables_param_act(XTF_ONE_ACTION, "CONNZEXTMARK", *flags & F_MARK);
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNZEXTMARK", "--xor-mark", optarg);
		info->mode   = XT_CONNMARK_SET;
		info->ctmark = value;
		info->ctmask = 0;
		*flags      |= F_MARK;
	...
	case 'n': /* --nfmask */
		if (!(*flags & F_SR_MARK))
			xtables_error(PARAMETER_PROBLEM, "CONNZEXTMARK: --save-mark "
			           "or --restore-mark is required for "
			           "--nfmask");
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNZEXTMARK", "--nfmask", optarg);
		info->nfmask = value;
		return true;
	...
	case 'c': /* --ctmask */
		if (!(*flags & F_SR_MARK))
			xtables_error(PARAMETER_PROBLEM, "CONNZEXTMARK: --save-mark "
			           ...
			           "--ctmask");
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNZEXTMARK", "--ctmask", optarg);
		info->ctmask = value;
		return true;
	...
	case 'm': /* --mask */
		if (!(*flags & F_SR_MARK))
			xtables_error(PARAMETER_PROBLEM, "CONNZEXTMARK: --save-mark "
			           ...
			           "--mask");
		if (!xtables_strtoui(optarg, NULL, &value, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "CONNZEXTMARK", "--mask", optarg);
...
static void connmark_tg_check(unsigned int flags)
{
	if (!flags)
		xtables_error(PARAMETER_PROBLEM,
		           "CONNZEXTMARK target: No operation specified");
}
                           ...
                           const struct xt_entry_target *target, int numeric)
{
	...
	case XT_CONNMARK_SET:
	    printf("CONNZEXTMARK set ");
	    print_mark(markinfo->mark);
	    print_mask("/", markinfo->mask);
	    printf(" ");
	    break;
	case XT_CONNMARK_SAVE:
	    printf("CONNZEXTMARK save ");
	    print_mask("mask ", markinfo->mask);
	    printf(" ");
	    break;
	case XT_CONNMARK_RESTORE:
	    printf("CONNZEXTMARK restore ");
	    print_mask("mask ", markinfo->mask);
	    break;
	default:
	    printf("ERROR: UNKNOWN CONNZEXTMARK MODE ");
	    break;
	}
}
                  ...
                  int numeric)
{
	...
	case XT_CONNMARK_SET:
		if (info->ctmark == 0)
			printf("CONNZEXTMARK and 0x%x ",
			       (unsigned int)(u_int32_t)~info->ctmask);
		else if (info->ctmark == info->ctmask)
			printf("CONNZEXTMARK or 0x%x ", info->ctmark);
		else if (info->ctmask == 0)
			printf("CONNZEXTMARK xor 0x%x ", info->ctmark);
		else if (info->ctmask == 0xFFFFFFFFU)
			printf("CONNZEXTMARK set 0x%x ", info->ctmark);
		else
			printf("CONNZEXTMARK xset 0x%x/0x%x ",
	...
	case XT_CONNMARK_SAVE:
		if (info->nfmask == UINT32_MAX && info->ctmask == UINT32_MAX)
			...
			printf("CONNZEXTMARK save mask 0x%x ", info->nfmask);
		else
			printf("CONNZEXTMARK save nfmask 0x%x ctmask ~0x%x ",
	...
	case XT_CONNMARK_RESTORE:
		if (info->ctmask == UINT32_MAX && info->nfmask == UINT32_MAX)
			...
			printf("CONNZEXTMARK restore mask 0x%x ", info->ctmask);
		else
			printf("CONNZEXTMARK restore ctmask 0x%x nfmask ~0x%x ",
