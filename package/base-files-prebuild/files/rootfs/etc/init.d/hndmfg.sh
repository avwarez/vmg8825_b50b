...
mfg_init()
{
    ...
    # Parse URL.
	# Zyxel there is no url paramiter to parse
	if [ $ZYXEL_MFG -ne 1 ]; then
		mfg_parse_url
	fi
...
mfg_mount_nand_nvram_fs()
{
	...
	ubiformat /dev/mtd$MTD -e 0 || error_exit "$0: ubiformat failed"
	...
	if [ $ZYXEL_MFG -ne 1 ]; then
		# Zyxel opel shell does not support brcm replace style
...
mfg_ubidetach_and_remount()
{
    if [ "$FLASHTYPE" == "NAND" ]; then
        echo "[$0]: Detaching MTD device $MTD from UBI and remount"
...
mfg_mount_nand_nvram_fs_only()
{
	...
	echo "[$0]: Mounting nvram UBI volume"
	mount -t ubifs $UBI:nvram $MFG_NVRAM_DIR || mfg_ubidetach_and_remount
...
mfg_program_nvram()
{
    ...
    then
		...
		# Zyxel Do not download the file cause there is no url.
		if [ $ZYXEL_MFG -ne 1 ]; then
			mfg_download_nvram_file
		fi
	...
	if [ $ZYXEL_MFG -ne 1 ]; then
		...
		# Zyxel: if misc1 is empty, copy FW nvram to misc1
		if [ ! -e $MFG_NVRAM_DIR/$MFG_NVRAM_FILE ] ; then
			echo "[$0]: misc1 is empty. copying nvram file to misc1"
			cp -a $MFG_NVRAM_WLAN_DIR/$MFG_NVRAM_FILE $MFG_NVRAM_DIR/$MFG_NVRAM_FILE
		fi
	fi
    fi
...
case "$1" in
    ...
    if [ ! -f $MFG_NVRAM_WLAN_DIR/$MFG_NVRAM_FILE ]; then
        # Zyxel Because only 43684 wifi project using new NVM file currently
        # So if there is no any NVM file in wlan folder, skip the script
        exit 0
    fi
